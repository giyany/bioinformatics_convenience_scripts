...
mkdir reports;
while read ind; do
         fastqc -t 12 -f fastq "$ind"_Lib1_R1.fq "$ind"_Lib1_R2.fq;
         java -jar ~/Programs/Trimmomatic-0.36/trimmomatic-0.36.jar PE -threads 12 -phred33 -trimlog "$ind"_trim_log "$ind"_Lib1_R1.fq "$ind"_Lib1_R2.fq "$ind"_Lib1_R1_paired.fq "$ind"_Lib1_R1_unpaired.fq "$ind"_Lib1_R2_paired.fq "$ind"_Lib1_R2_unpaired.fq ILLUMINACLIP:TruSeq3-PE-2.fa:2:30:10 LEADING:20 TRAILING:20 SLIDINGWINDOW:4:15 MINLEN:50;
         mv "$ind"_trim_log reports;
         fastqc -t 12 "$ind"_Lib1_R1_paired.fq "$ind"_Lib1_R2_paired.fq;
 done < $indlist

#awkward code for collecting info about read trimming
 while read ind; do
         unzip "$ind"_Lib1_R1_fastqc.zip;
         echo "R1 before trim" >> reports/"$ind"_Trim_Sequences_Count.txt;
         less "$ind"_Lib1_R1_fastqc/fastqc_data.txt | grep "Total Sequences" >> reports/"$ind"_Trim_Sequences_Count.txt;
         rm -r "$ind"_Lib1_R1_fastqc;
         mv "$ind"_Lib1_R1_fastqc.zip reports;
         unzip "$ind"_Lib1_R2_fastqc.zip;
         echo "R2 before trim" >> reports/"$ind"_Trim_Sequences_Count.txt;
         less "$ind"_Lib1_R2_fastqc/fastqc_data.txt | grep "Total Sequences" >> reports/"$ind"_Trim_Sequences_Count.txt;
         rm -r "$ind"_Lib1_R2_fastqc;
         mv "$ind"_Lib1_R2_fastqc.zip reports;
         unzip "$ind"_Lib1_R1_paired_fastqc.zip;
         echo "R1 after trim" >> reports/"$ind"_Trim_Sequences_Count.txt;
         less "$ind"_Lib1_R1_paired_fastqc/fastqc_data.txt | grep "Total Sequences" >> reports/"$ind"_Trim_Sequences_Count.txt;
         rm -r "$ind"_Lib1_R1_paired_fastqc;
         mv "$ind"_Lib1_R1_paired_fastqc.zip reports;
         unzip "$ind"_Lib1_R2_paired_fastqc.zip;
         echo "R2 after trim" >> reports/"$ind"_Trim_Sequences_Count.txt;
         less "$ind"_Lib1_R2_paired_fastqc/fastqc_data.txt | grep "Total Sequences" >> reports/"$ind"_Trim_Sequences_Count.txt;      
         rm -r "$ind"_Lib1_R2_paired_fastqc;
         mv "$ind"_Lib1_R2_paired_fastqc.zip reports;
         mv "$ind"_Lib1_R1_fastqc.html reports;
         mv "$ind"_Lib1_R2_fastqc.html reports;
         mv "$ind"_Lib1_R1_paired_fastqc.html reports;
         mv "$ind"_Lib1_R2_paired_fastqc.html reports;
 done < $indlist


while read ind; do
	do echo "bowtie aligning $ind";
	bowtie2 --very-sensitive-local -x /media/computer/097c7584-78ef-4295-907e-df3c187190c3/Science/Data/Refgenome/Tfas_bowtieindex -1 "$ind"_Lib1_R1_paired.fq -2 "$ind"_Lib1_R2_paired.fq -S "$ind"_aligned_Tfasc.sam -p 14;
...
# Call variants (code from cluster scripts)
